...
| `auth.allow_origins`          | `JUMPERS_AUTH_ALLOW_ORIGINS`          | Explicit origins allowed for auth actions. Supports single-wildcard patterns like `http://localhost:*`; `*` is rejected when auth is enabled. | `[]`           | `["http://localhost:*"]` |
